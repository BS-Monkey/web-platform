...
const Pipeline = {
  async add(): Promise<Pipeline> {
    const organizationId = API.Organization_v2.getCurrentId();

    try {
      const response = await http
        .post(`pipelines`, {}, { params: { organization_id: organizationId }});
   ...
   */
  async update(pipeline: Pipeline): Promise<void> {
    const organizationId = API.Organization_v2.getCurrentId();
    try {
      await http
        ...
        .then(() => {
          API.UI.addNotification({ type: 'save', text: 'Pipeline saved.' });
        })
        .catch((error) => {
          API.UI.addNotification({ type: 'danger', text: 'Error updating pipeline.' });
        });
    } catch (error) {
      throw error;
    }
