    ...
    <div v-if="pipelines && pipelines.length">
      <div class="pipeline" v-for="pipeline in pipelines" :key="pipeline.id">
        <router-link
          ...
          :disabled="disabledAccess"
          class="button default outline right"
        >
      ...
      <button class="add block" v-if="!disabledAccess" @click="createPipeline()">+ New Pipeline</button>
    </div>
...
export default class PipelineIndex extends Vue {
  ...
  private createPipeline(): void {
    API.Pipeline.add().then((res) => {
      router.push(`pipelines/settings/${res.id}`);
    });
  }
