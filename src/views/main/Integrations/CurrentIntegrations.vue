...
export default class CurrentIntegrations extends Vue {
  ...
  private remove(index: number, integration: Integration<All>): void {
    this.setDeleteBanners(index, false);
    API.Integration.remove(integration).then(() => {
        this.$emit('deleteIntegration');
    });
  }

  private isCMI(integration: Integration<All>): boolean {
    const cmi = ['aws', 'gcp', 'netapp', 'azure', 'agent', 'zenoss', 'windows', 'sqlserver'];
  ...
  private isThresholdSupported(integration: Integration<All>): boolean {
    const supported = ['aws', 'gcp', 'netapp', 'azure', 'agent', 'zenoss', 'globe_billing', 'windows', 'sqlserver'];
